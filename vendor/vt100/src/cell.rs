...
pub struct Cell {
    ...
    // OSC 8 hyperlink target for this cell, if any (ruckus patch). Arc so the
    // Screen/Parser stay Send + Sync.
...
impl Cell {
    ...
    pub(crate) fn set_hyperlink(&mut self, link: Option<std::sync::Arc<str>>) {
        self.hyperlink = link;
    }

    /// The OSC 8 hyperlink target for this cell, if the app marked one (ruckus).
    #[must_use]
