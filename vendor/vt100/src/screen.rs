...
pub struct Screen {
    ...
    title: String,
    icon_name: String,
    // Current OSC 8 hyperlink target; stamped onto each cell drawn (ruckus).
...
impl vte::Perform for Screen {
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], _bel_terminated: bool) {
        match (params.get(0), params.get(1)) {
            ...
            // OSC 8 ; params ; URI  — hyperlinks (ruckus patch). Empty URI clears.
            (Some(&b"8"), _) => {
                let uri: String = if params.len() >= 3 {
                    params[2..]
                        .iter()
                ...
                self.current_hyperlink =
                    if uri.is_empty() { None } else { Some(std::sync::Arc::from(uri.as_str())) };
            }
            _ => {
                if log::log_enabled!(log::Level::Debug) {
                    log::debug!(
